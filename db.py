...
def init_db():
    try:
        ...
        with conn.cursor() as cursor:
            ...
            cursor.execute("CREATE DATABASE IF NOT EXISTS tests CHARACTER SET utf8mb4 COLLATE utf8mb4_unicode_ci")
        ...
        with conn.cursor() as cursor:
            ...
            if not check_index_exists(cursor, 'users', 'idx_user_email'):
                cursor.execute("CREATE INDEX idx_user_email ON users(email)")
            if not check_index_exists(cursor, 'tests', 'idx_test_creator'):
                cursor.execute("CREATE INDEX idx_test_creator ON tests(creator_id)")
            if not check_index_exists(cursor, 'questions', 'idx_question_test'):
                cursor.execute("CREATE INDEX idx_question_test ON questions(test_id)")
            if not check_index_exists(cursor, 'test_attempts', 'idx_attempt_user_test'):
                cursor.execute("CREATE INDEX idx_attempt_user_test ON test_attempts(user_id, test_id)")
